...
public class PlayerPermissions {
    ...
    /**
     * Returns true if player is operator. Can only return true if called server-side.
     */
    public static boolean isOp(EntityPlayer entityPlayer) {
        if (!entityPlayer.worldObj.isRemote) {
            return ((EntityPlayerMP) entityPlayer).mcServer.getConfigurationManager()
    ...
    public static boolean hasElevatedPermission(IProtected object, EntityPlayer entityPlayer,
        ...
        if (entityPlayer.worldObj.isRemote && Minecraft.getMinecraft().isSingleplayer()) { // Check if client is playing
                                                                                           // singleplayer
            return true;
        } else if (!entityPlayer.worldObj.isRemote && MinecraftServer.getServer().isSinglePlayer()) { // Check if server
                                                                                                      // is integrated
                                                                                                      // (singleplayer)
            return true;
