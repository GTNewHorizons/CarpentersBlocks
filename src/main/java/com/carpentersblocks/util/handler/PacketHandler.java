...
public class PacketHandler {
    ...
    public void onServerPacket(ServerCustomPacketEvent event) throws IOException {
        ...
        if (packetId < packetCarrier.size()) {
            try {
                ICarpentersPacket packetClass =
                        (ICarpentersPacket) packetCarrier.get(packetId).newInstance();
    ...
    public static void sendPacketToServer(ICarpentersPacket packet) {
        ...
        CarpentersBlocks.channel.sendToServer(
                new FMLProxyPacket(new C17PacketCustomPayload(CarpentersBlocks.MODID, buffer)));
    }
}
